...
public class WinMenu extends Menu {
    ...
    public WinMenu(WarriorsGame game) {
        this.game = game;
    }
    ...
    public void draw(Canvas canvas) {
        ...
        canvas.drawGradientRect(startX-50, startY -130, 400, 280, new Color(44, 119, 0, 47), new Color(0, 0, 0, 68));

        canvas.setFont(new Font("Arial", Font.BOLD, 50));
        String gameOverText = "YOU WIN";
        int win = canvas.getGraphics().getFontMetrics().stringWidth(gameOverText);
        int gameOverX = (800 - win) / 2;
        int gameOverY = startY - 50;

        canvas.drawString(gameOverText, gameOverX, gameOverY, Color.GREEN);
